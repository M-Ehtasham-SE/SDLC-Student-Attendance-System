...
export default function LoginPage() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (String(role).toLowerCase() === "admin") {
        ...
        // If an activeTeacher is set, only that teacher can sign in
        try {
          const rawActiveT = localStorage.getItem("activeTeacher")
          const activeT = rawActiveT ? JSON.parse(rawActiveT) : null
          if (activeT && String(activeT.username).toLowerCase() !== uname.toLowerCase()) {
            setError(`Teacher portal access is restricted to '${activeT.username}'. Please contact admin.`)
            setIsLoading(false)
            return
          }
        } catch (e) {
  ...
  const handleSetPassword = async () => {
    ...
    try {
      ...
      // persist session only after active-role checks completed
      try {
        if (String(users[targetIndex].role).toLowerCase() === "teacher") {
          const rawActiveT = localStorage.getItem("activeTeacher")
          const activeT = rawActiveT ? JSON.parse(rawActiveT) : null
          if (activeT && String(activeT.username).toLowerCase() !== String(users[targetIndex].username).toLowerCase()) {
            setSetPwError(`Teacher portal access is restricted to '${activeT.username}'. Please contact admin.`)
            setIsSetting(false)
            return
          }
        }
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="text-center space-y-3">
            ...
            <p className="text-lg text-muted-foreground">
              Track attendance, manage results, and inspire student success with LearnSync.
            </p>
          </div>
